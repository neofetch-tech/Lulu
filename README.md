...
The project provides a desktop interface and runtime layer for managing, loading, and interacting with local models without requiring a remote inference service.
...
The project combines a native inference backend with a higher-level application layer and a desktop interface.
...
The desktop layer and inference layer are intentionally separated so that the runtime can evolve independently from the user interface.
